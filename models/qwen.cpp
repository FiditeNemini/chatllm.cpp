...
namespace chatllm::qwen::v3_vl
{
    ...
    protected:
        ...
        virtual void prepare_pos_tensor(const std::vector<int> &input_ids, const GenerationConfig &gen_config);
    public:
    ...
    void ConditionalGeneration::prepare_pos_tensor(const std::vector<int> &input_ids, const GenerationConfig &gen_config)
    {
        const int image_id_start = config.vocab_size;
        const int length = (int)input_ids.size();
...
namespace chatllm::qwen::v3_vl_emb
{
    ...
    public:
        ...
        void encode_embedding(const Content &input, std::vector<int> &ids, EmbeddingPurpose purpose) const override
        {
            Messages messages;
    ...
    void ConditionalGeneration::embedding(const GenerationConfig &gen_config, const std::vector<int> &input_ids,
                                std::vector<float> &embedding)
    {
        prepare_pos_tensor(input_ids, gen_config);

        v3::ConditionalGeneration::embedding(gen_config, input_ids, embedding);
    }
}
